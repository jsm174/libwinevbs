        ...
        BSTR *pBstrDocString, DWORD *pdwHelpContext, BSTR *pBstrHelpFile)
{
    ...
    if (memid == MEMBERID_NIL) {
        if (pBstrName)
            *pBstrName = This->disp->desc ? SysAllocString(This->disp->desc->name) : NULL;
        return S_OK;
    }
...
static HRESULT alloc_vbdisp_typeinfo(vbdisp_t *disp, ITypeInfo **out)
{
    ...
    type_info->ref = 1;
    type_info->disp = disp;
    IDispatchEx_AddRef(&disp->IDispatchEx_iface);

    *out = &type_info->ITypeInfo_iface;
    return S_OK;
}
#endif
...
#else
    return alloc_vbdisp_typeinfo(This, ppTInfo);
#endif
}
